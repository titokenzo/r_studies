# Funções: Família Apply - Uma forma elegante de fazer loops
...
# vapply() - simalar a sapply(), com valor de retorno modificado
# rapply() - similar a lapply()
# eapply() - gera uma lista
# mapply() - similar a sapply(), multivariada
# by - (similar a família apply) agregação  com os dados

# Se você estiver trabalhando com objetos:
# list, nuumeric, character (list/vector) => sapply ou lapply
...
# Calculando cada elemento da lsita com sapply()
?sapply
sapply(lista1, sum)
...
escola$Media <- apply(escola[,c(2,3,4)], 1, mean) #todas as linhas (,)
escola
